...
def test_overwrite_flag_false_skips_tattooed_files():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert content_after_first == content_after_second, "File should not be overwritten when overwrite=False"
...
def test_overwrite_flag_true_replaces_tattoos():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert content_after_first != content_after_second, "File should be overwritten when overwrite=True"
        assert original_code in content_after_second, "Original code should still be preserved"
...
def test_overwrite_preserves_shebang():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        lines = content.split('\n')
        assert lines[0] == "#!/usr/bin/env python3", "Shebang should still be preserved after overwrite"
...
def test_default_behavior_is_no_overwrite():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert content_after_first == content_after_second, "Default behavior should not overwrite"
