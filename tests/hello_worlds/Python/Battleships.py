    ...
    for x in range(len(board)):
        ...
        for y in range(len(board[x])):
            ...
            str_row += (
                board[x][y] == 0
                and "[ ]"
                or (board[x][y] == 1 and "[S]" or (board[x][y] == 2 and "[X]" or "[M]"))
            )
...
def print_board_exclusion(board, exclude_num):
    ...
    for x in range(len(board)):
        ...
        for y in range(len(board[x])):
            ...
            # value 3: hit (miss), '[M]'
            str_row += (
                (board[x][y] == exclude_num or board[x][y] == 0)
                and "[ ]"
                or (board[x][y] == 1 and "[S]" or (board[x][y] == 2 and "[X]" or "[M]"))
            )
        # Actually prints the nth row of the board.
    ...
    while True:
        ...
        if player_ships == 0:
            ...
            if not FOUNDSHIP:
                ...
                while (
                    player_board_copy[x][y] == 2 or player_board_copy[x][y] == 3
                ) or ((x, y) in ret):
                    x, y = (random.randint(1, 10), random.randint(1, 10))
                    x, y = clamp(y - 1, 0, 9), clamp(x - 1, 0, 9)
...
def start_game():
    ...
    while True:
        ...
        if status:
            ...
            print(
                msg == "ENEMY"
                and "You have lost to the enemy!"
                or "You have won against the enemy!"
            )
