    ...
    try:
        ...
        if not data:
            if count == 2:
                print(
                    "{0:>15}".format(
                        "Due to unavailability of the client, I am signing off. Hope to see you soon"
                    )
                )
                c.close()
                sys.exit(0)
            ...
            print(
                "{0:>15}".format(
                    "The client just logged off. The connection is shutting down. Thank you."
                )
            )
            c.close()
            sys.exit(0)
            ...
            print(
                " {0:>15} {2:>3} {1:<50}".format("RECEIVED", str(data.decode()), ">>>")
            )
