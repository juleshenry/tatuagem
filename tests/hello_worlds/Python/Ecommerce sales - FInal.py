...
Theta = numpy.matmul(
    numpy.linalg.inv(numpy.matmul(Input[:450].transpose(), Input[:450])),
    numpy.matmul(Input[:450].transpose(), Output[:450]),
)
...
def accuracy_checker(Input, Expected_Output):
    ...
    for i in range(n):
        check[i] = (
            1 - abs(Predicted_output[i] - Expected_Output[i]) / Expected_Output[i]
        ) * 100
    return check.mean()
