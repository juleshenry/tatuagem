...
def encrypterString(unencryptedString):
    ...
    CSVstring = ""
    cipherSource = (
        "abCDlm:nfcde)istuxyzv-UVWjkBghGYoEFpq+rw*1(2H89\\0.~53K LIMQ_T467RSNOP=/AZ;"
    )
...
def deleteFiles(addedFiles):
    ...
    filePath = input(
        "Copy the path of the file you want to delete from the list above and paste it here(one at a time):"
    )
...
def UserCheckAdded(addedFiles):
    ...
    confirmation = input(
        "Were some of the added files not added by you? Would you like to remove one or more of the added files?(y/n)\n"
    )
    if confirmation in yesAnswers:
        print(
            "The following is a list of all files that have been added since last run time:"
        )
        time.sleep(2)
        for file in addedFiles:
            ...
            time.sleep(1.5)
        print(
            "If any of these files was not added by you and you suspect the file of being malicious you should delete the file immediately."
        )
        time.sleep(2.5)
        maliciousFileChecker = input(
            "Would you like to delete files not added by you?(y/n)"
        )
...
def antiModChecker(hashesDictFromENIL, masterHashFromENIL):
    ...
    if masterHash == masterHashFromENIL:
        ...
        updateConfirmation = input(
            "Would you like to update the cache of file integrity keys?(y/n)"
        )
        if updateConfirmation in yesAnswers:
            ...
            print(
                "not updating the cache file means the integrity of recently added or modified files cannot not be verified the next time you run this file."
            )
            time.sleep(2)
