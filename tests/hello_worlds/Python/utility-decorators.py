    ...
    def decorator(function):
        ...
        def inner(*args, **kwargs):
            ...
            if format is True:
                ...
                kwargs_str = ", ".join(
                    ("{}={}".format(str(i), str(kwargs[i])) for i in kwargs)
                )
                string = "{}({}{}{}) -> {}".format(
                    name,
                    args_str,
            ...
            if order == "any":
                for i in args + tuple(kwargs.values()):
                    if type(i) not in limit:
                        raise TypeError(
                            "Argument Type ({}) is Not An Accepted Type".format(
                                type(i).__name__
                            )
                        )
            elif order == "restrict":
                ...
                for i in args + tuple(kwargs.values()):
                    if type(i) not in local_limit:
                        raise TypeError(
                            "Argument Type ({}) is Not An Accepted Type. This may be due to previous arguments exhausting the accepted list".format(
                                type(i).__name__
                            )
                        )
                    else:
                ...
                for i in zip((args + tuple(kwargs.values())), limit):
                    if type(i[0]) is not i[1]:
                        raise TypeError(
                            "Argument Type ({}) is Not An Accepted Type. This may be due to incorrect order of arguments".format(
                                type(i[0]).__name__
                            )
                        )
    ...
    def decorator(function):
        ...
        def inner(*args, **kwargs):
            ...
            if not type(return_value) in limit:
                raise TypeError(
                    "Function Returned A Non-Valid Return Value: {}".format(
