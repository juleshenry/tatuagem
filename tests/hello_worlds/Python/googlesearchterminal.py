...
parser.add_option(
    ...
    type="int",
    dest="results",
    help="display specified number of results, default is 10.",
)
parser.add_option(
    "-v",
    "--view",
    ...
    type="int",
    dest="view",
    help="view page by index number.",
)
...
for arg in args:
    option += arg + "+"
if len(option) == 0:
    parser.error("please enter a search term")

from urllib import FancyURLopener


class MyOpener(FancyURLopener):
    version = ""  # google doesn't like the user agent so send blank it blank
...
for s in entry:
    ...
    if options.results == counter:
        break
    counter += 1
file = open("/tmp/google", "w")
for entries in range(1, len(dict) + 1):
    file.write(dict[entries] + "\n")
