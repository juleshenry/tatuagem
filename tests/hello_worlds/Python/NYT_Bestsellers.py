...
while bool == True:
    ...
    if category == "Fiction":
        print("\n~Fiction~")
        print(
            "Combined Print & E-Book Fiction, Hard Cover Fiction, Paperback Trade Fiction"
        )
    ...
    # Nonfiction
        ...
        print("\n~Nonfiction~")
        print(
            "Combined Print & E-Book Nonfiction, Hardcover Nonfiction, Paperback Nonfiction, Advice, How-to & Miscellaneous"
        )
        type = str(input("Select a type: "))
...
r = requests.get(
    "http://api.nytimes.com/svc/books/v3/lists/current/"
    + categoryType
    + ".json?&api-key="
    + apiKey
)
...
for x in range(index):
    print(
        str(x + 1)
        + ". "
        + str(data["results"]["books"][x]["title"])
        + " by "
        + str(data["results"]["books"][x]["author"])
    )
