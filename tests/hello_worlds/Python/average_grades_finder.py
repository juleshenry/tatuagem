...
# program
def grades(x):
    return {"A": 10, "AB": 9, "B": 8, "BC": 7, "C": 6, "CD": 5, "D": 4}.get(x, 0)
...
def find_grades1(s):
    ...
    for i in range(len(sorted_roll_dict)):
        if students.__contains__(sorted_roll_dict[i][0]) and grade.__contains__(
            sorted_roll_dict[i][0]
        ):
            final_list.append(
                "{}~{}~{}".format(
                    sorted_roll_dict[i][0],
                    students[sorted_roll_dict[i][0]],
                    grade[sorted_roll_dict[i][0]],
                )
            )
        else:
            final_list.append(
                "{}~{}~{}".format(
                    sorted_roll_dict[i][0], students[sorted_roll_dict[i][0]], 0
                )
            )
    return "\n".join(final_list)
...
def find_grade(index, s):
    ...
    for i in range(index + 1, len(s) - 1):
        ...
        if dic1.__contains__(s[i].split("~")[-2]):
            dic1[s[i].split("~")[-2]] = dic1[s[i].split("~")[-2]] + grades(
                s[i].split("~")[-1]
            )
        else:
    ...
    for j in range(len(roll_nos)):
        dic1[roll_nos[j]] = round(
            dic1[roll_nos[j]] / repeated_rollnos.count(roll_nos[j]), 2
        )
