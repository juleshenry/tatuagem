...
th = np.linspace(-np.pi / 13, 2 * np.pi * nspiral, 1000)
# angle
(x, y) = spiral(th)
...
s = dict(x=-x + x[0], y=y - y[0] + yshift, line=dict(color="white", width=3))
...
hm = dict(
    x=np.sort(xe), y=np.sort(ye) + yshift, z=z, type="heatmap", colorscale="Viridis"
)
