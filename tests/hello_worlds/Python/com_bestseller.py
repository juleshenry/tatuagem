...
for j in range(0, len(pageURL)):
    ...
    for i in range(0, len(urltxt)):
        string = urltxt[i].find("div").find("a", {"class": "a-link-normal"})["href"]
        bookURL.append("https://www.amazon.com" + string)
        author_type1.append(
            urltxt[i].find("div", {"class": "a-row a-size-small"}).find("a")
        )
        author_type2.append(
            urltxt[i].find("div", {"class": "a-row a-size-small"}).find("span")
        )
        price.append(urltxt[i].find("span", {"class": "p13n-sc-price"}))
        temp = urltxt[i].find("div", {"aria-hidden": "true"}).contents[0].rsplit()
        name.append(" ".join(temp))
        ratings.append(urltxt[i].find("a", {"class": "a-size-small a-link-normal"}))
