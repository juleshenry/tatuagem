...
dict = unpickle(
    "/Users/wuzhengyu/Desktop/github/AI-mini-project/AlexNet/cifar10/data_batch_1"
)
dict1 = unpickle(
    "/Users/wuzhengyu/Desktop/github/AI-mini-project/AlexNet/cifar10/test_batch"
)
...
class AlexNet(tf.keras.Model):
    def __init__(self):
        ...
        self.lrn1 = tf.keras.layers.BatchNormalization(
        ...
        self.lrn2 = tf.keras.layers.BatchNormalization(
            axis=-1, momentum=0.99, epsilon=0.001, center=True, scale=True
        )
        self.pool2 = tf.keras.layers.MaxPool2D(pool_size=[2, 2], strides=2)
        self.conv3 = tf.keras.layers.Conv2D(
            filters=192, kernel_size=[3, 3], padding="same", activation=tf.nn.relu
        )
...
X_placeholder = tf.placeholder(
    dtype=tf.float32, shape=(batch_size, 32 * 32 * 3), name="input"
)
...
with tf.Session() as sess:
    ...
    for batch_index in range(300):
        ...
        _, result = sess.run(
            [train_op, merged], feed_dict={X_placeholder: X, y_placeholder: y}
        )
    ...
    print(
        "Test accuracy: %f"
        % (np.sum(y_pred_np == data_loader.eval_labels) / num_eval_samples)
    )
