...
RomanValueDict = {"I": 1, "V": 5, "X": 10, "L": 50, "C": 100, "D": 500}
...
RomanHndsTriplet = {
    ...
    "q": "D",
    "d": "M",
}  # Should never have to use the 'M', though
...
if len(sys.argv) == 1:
    ...
    manualMode = True
    mmPrint(
        "No argument detected. Entering manual mode. Please enter input numeral (Roman or Arabic)."
    )
...
if len(sys.argv) > 2:
    print(
        "Error: Invalid number of arguments. Please input one Roman or Arabic numeral."
    )
    sys.exit()
...
if containsRoman:
    ...
    for index in range(len(RValue)):
        ...
        if iFollowingNumerals >= 1:  # At least one numeral following
            if (
                RomanValueDict[letter] < RomanValueDict[RValue[index + 1]]
            ):  # IV, IX, IC, VX, etc.
                if RomanValueDict[letter] in (1, 10, 100):
                    if RomanValueDict[RValue[index + 1]] in (
                        (RomanValueDict[letter] * 5),
                        (RomanValueDict[letter] * 10),
                    ):
                        if (
                            iFollowingNumerals >= 2
                            and RomanValueDict[letter]
                            == RomanValueDict[RValue[index + 2]]
                        ):  # IVI, XCX, etc.
            ...
            if iFollowingNumerals >= 2:  # At least two numerals following
                if RomanValueDict[letter] == RomanValueDict[
                    RValue[index + 1]
                ] and RomanValueDict[RValue[index + 2]] in (
                    (RomanValueDict[letter] * 5),
                    (RomanValueDict[letter] * 10),
                ):  # IIV, XXC, etc.
                ...
                if iFollowingNumerals >= 3:  # At least three numerals following
                    if (
                        RomanValueDict[letter]
                        == RomanValueDict[RValue[index + 1]]
                        == RomanValueDict[RValue[index + 2]]
                        == RomanValueDict[RValue[index + 3]]
                    ):  # IIII, XXXX, etc.
