...
class GetPostFromUrl:
    ...
    def transform(self, rss_url, result_data):
        ...
        for each_post in result_data["entries"]:
            ...
            if "description" in each_post:
                output["post_description"] = BeautifulSoup(
                    each_post["description"], "html.parser"
                ).text
            else:
            ...
            if "summary_detail" in each_post and bool(
                BeautifulSoup(
                    each_post["summary_detail"]["value"], "html.parser"
                ).find()
            ):
                image_details = BeautifulSoup(
                    each_post["summary_detail"]["value"], "html.parser"
                )
                if image_details.find_all("img"):
                    ...
                    output["post_image_url"] = ""
            elif "content" in each_post and bool(
                BeautifulSoup(each_post["content"][0]["value"], "html.parser").find()
            ):
                image_details = BeautifulSoup(
                    each_post["content"][0]["value"], "html.parser"
                )
                if image_details.find_all("img"):
                    for every_image_tag in image_details.find_all("img"):
                        if every_image_tag.get("src"):
    ...
    def store_results(self, output):
        filename = (
            "GetPostFromUrl"
            + ("_".join(str(datetime.now()).split(" "))).replace(":", "-")
            + ".json"
        )
...
if __name__ == "__main__":
    ...
    parameters = {}
    parameters["urls_of_sites"] = [
        "https://www.google.co.in/alerts/feeds/00658181146524679235/12925205973984843373"
    ]
    obj = GetPostFromUrl()
